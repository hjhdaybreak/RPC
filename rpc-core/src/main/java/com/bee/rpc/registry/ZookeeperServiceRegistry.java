    ...
    public InetSocketAddress lookupService(String serviceName, int requestHashCode) {
        try {
            ...
            if (serviceAndInetMap == null) {
                ...
                List<String> zooKeeperChildren = zooKeeper.getChildren(SERVER_PATH_PREFIX + SPILT + serviceName, null);
                for (String servicePath : zooKeeperChildren) {
                    // rehash
                    consistentHashingWithVirtualNode.addGroup(servicePath);
                    String checkedChildrenPath = SERVER_PATH_PREFIX + SPILT + serviceName + SPILT + servicePath;
                    byte[] serviceIpAndPort = zooKeeper.getData(checkedChildrenPath, null, null);
                    String[] data = new String(serviceIpAndPort).split(":");
                    serviceAndInetMap.put(servicePath, new InetSocketAddress(data[0], Integer.parseInt(data[1])));
                }
                //刷新一致性hash环
            ...
            String path = consistentHashingWithVirtualNode.getServer(requestHashCode);
